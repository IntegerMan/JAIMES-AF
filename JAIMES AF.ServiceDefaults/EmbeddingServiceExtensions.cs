...
public static class EmbeddingServiceExtensions
{
    ...
    {
        // Bind configuration
        EmbeddingModelOptions options = configuration.GetSection(sectionName).Get<EmbeddingModelOptions>() ??
        ...
        switch (options.Provider)
        {
                ...
                services.AddSingleton<IEmbeddingGenerator<string, Embedding<float>>>(sp =>
                {
                    ...
                    if (string.IsNullOrWhiteSpace(opts.Endpoint))
                        throw new InvalidOperationException(
                            "Azure OpenAI endpoint is not configured. Set EmbeddingModel:Endpoint.");

                    if (string.IsNullOrWhiteSpace(opts.Name))
                        throw new InvalidOperationException(
                            "Azure OpenAI deployment name is not configured. Set EmbeddingModel:Name.");

                    if (opts.Auth == AuthenticationType.ApiKey && string.IsNullOrWhiteSpace(opts.Key))
                        throw new InvalidOperationException(
                            "Azure OpenAI API key is not configured. Set EmbeddingModel:Key.");

                    logger.LogDebug(
                        "Creating Azure OpenAI embedding generator with deployment {Deployment} at {Endpoint} with auth type {AuthType}",
                        opts.Name,
                        opts.Endpoint,
                        opts.Auth);
            ...
            case ProviderType.OpenAi:
                services.AddSingleton<IEmbeddingGenerator<string, Embedding<float>>>(sp =>
                {
                    ...
                    if (string.IsNullOrWhiteSpace(opts.Name))
                        throw new InvalidOperationException(
                            "OpenAI model name is not configured. Set EmbeddingModel:Name.");
                    ...
                    if (string.IsNullOrWhiteSpace(opts.Key))
                        throw new InvalidOperationException(
                            "OpenAI API key is not configured. Set EmbeddingModel:Key.");
            ...
            default:
                services.AddSingleton<IEmbeddingGenerator<string, Embedding<float>>>(sp =>
                {
                    ...
                    if (string.IsNullOrWhiteSpace(opts.Endpoint))
                        throw new InvalidOperationException(
                            "Ollama endpoint is not configured. Set EmbeddingModel:Endpoint.");

                    if (string.IsNullOrWhiteSpace(opts.Name))
                        throw new InvalidOperationException(
                            "Ollama model name is not configured. Set EmbeddingModel:Name.");
