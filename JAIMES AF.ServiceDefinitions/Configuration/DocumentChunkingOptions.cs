...
public class DocumentChunkingOptions
{
    ...
    public int TokenLimit { get; set; } =
        1024; // Max tokens per chunk - targeting paragraph-sized chunks (small to medium paragraphs)

    public int BufferSize { get; set; } =
        2; // Sentences added before/after current sentence during embedding (increased for better context)

    public string ThresholdType { get; set; } =
        "Percentile"; // Options: Percentile, StandardDeviation, InterQuartile, Gradient

    public double ThresholdAmount { get; set; } =
        90.0; // E.g., 90 for Percentile (lower = larger chunks), 3.0 for StandardDeviation
