...
public class SearchRulesEndpointTests : EndpointTestBase
{
        ...
        Factory = new WebApplicationFactory<ApiServiceProgram>()
            .WithWebHostBuilder(builder =>
            {
                ...
                builder.ConfigureServices(services =>
                {
                    ...
                    ServiceDescriptor? rulesSearchDescriptor =
                        services.FirstOrDefault(d => d.ServiceType == typeof(IRulesSearchService));
                    if (rulesSearchDescriptor != null) services.Remove(rulesSearchDescriptor);
    ...
    public async Task SearchRulesEndpoint_WithValidQuery_ReturnsResults()
    {
        ...
        HttpResponseMessage response =
            await Client.PostAsJsonAsync("/rules/search", request, TestContext.Current.CancellationToken);

        // Assert
        response.StatusCode.ShouldBe(HttpStatusCode.OK);
        SearchRulesResponse? payload =
            await response.Content.ReadFromJsonAsync<SearchRulesResponse>(TestContext.Current.CancellationToken);
    ...
    public async Task SearchRulesEndpoint_WithRulesetFilter_ReturnsFilteredResults()
    {
        ...
        _mockRulesSearchService
            .Setup(s => s.SearchRulesDetailedAsync("dnd5e",
                "spell casting",
                It.IsAny<bool>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(new SearchRulesResponse {Results = expectedResults});
    ...
    public async Task SearchRulesEndpoint_ReturnsTop5ResultsOrderedByRelevancy()
    {
        ...
        SearchRuleResult[] expectedResults = new[]
        {
            new SearchRuleResult
            {
                Text = "Result 1", DocumentId = "doc1", DocumentName = "Doc1.pdf", RulesetId = "dnd5e",
                EmbeddingId = "1", ChunkId = "chunk1", Relevancy = 0.99
            },
            new SearchRuleResult
            {
                Text = "Result 2", DocumentId = "doc2", DocumentName = "Doc2.pdf", RulesetId = "dnd5e",
                EmbeddingId = "2", ChunkId = "chunk2", Relevancy = 0.95
            },
            new SearchRuleResult
            {
                Text = "Result 3", DocumentId = "doc3", DocumentName = "Doc3.pdf", RulesetId = "dnd5e",
                EmbeddingId = "3", ChunkId = "chunk3", Relevancy = 0.90
            },
            new SearchRuleResult
            {
                Text = "Result 4", DocumentId = "doc4", DocumentName = "Doc4.pdf", RulesetId = "dnd5e",
                EmbeddingId = "4", ChunkId = "chunk4", Relevancy = 0.85
            },
            new SearchRuleResult
            {
                Text = "Result 5", DocumentId = "doc5", DocumentName = "Doc5.pdf", RulesetId = "dnd5e",
                EmbeddingId = "5", ChunkId = "chunk5", Relevancy = 0.80
            }
        };
    ...
    public async Task SearchRulesEndpoint_WithNoResults_ReturnsEmptyArray()
    {
        // Arrange
        _mockRulesSearchService
            .Setup(s => s.SearchRulesDetailedAsync(null,
                "nonexistent",
                It.IsAny<bool>(),
                It.IsAny<CancellationToken>()))
