...
public partial class PromptImproverWizard : IDisposable
{
    ...
    private bool _isGeneratingTools;
    ...
    private string _lastAutoGeneratedPrompt = string.Empty;
    ...
    private bool HasAnyInsights => !string.IsNullOrEmpty(_feedbackInsights) ||
                                   ...
                                   !string.IsNullOrEmpty(_toolInsights);
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    await InvokeAsync(async () =>
                    {
                        ...
                        var toolsTask = GenerateToolInsightsAsync(token);
                        await Task.WhenAll(feedbackTask, metricsTask, sentimentTask, messagesTask, toolsTask);
                    });
                }
    ...
    private void ResetState()
    {
        ...
        _editableInputPrompt = string.Empty;
        _lastAutoGeneratedPrompt = string.Empty;
        _inputPromptExpanded = true;
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                if (result?.Success == true)
                {
                    ...
                    UpdateEditablePromptIfUnchanged();
                }
                else
                {
    ...
    private async Task GenerateToolInsightsAsync(CancellationToken cancellationToken = default)
    {
        _isGeneratingTools = true;
        StateHasChanged();
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                if (result?.Success == true)
                {
                    _toolInsights = result.Insights;
                    UpdateEditablePromptIfUnchanged();
                }
                else
                {
                    Snackbar.Add(result?.Error ?? "Failed to generate tool insights", Severity.Error);
                }
            }
            else
            {
                if (cancellationToken.IsCancellationRequested) return;
                Snackbar.Add("Failed to generate tool insights", Severity.Error);
            }
        }
        catch (OperationCanceledException)
        {
    ...
    private async Task GenerateImprovedPromptAsync()
    {
        ...
        try
        {
            var request = new GenerateImprovedPromptRequest
            {
                ...
                ManualInstructions = _editableInputPrompt == _lastAutoGeneratedPrompt ? null : _editableInputPrompt
            };
    ...
    private string GetFullPromptPreview()
    {
        ...
        if (!string.IsNullOrWhiteSpace(_toolInsights))
        {
            sb.AppendLine("## Insights from Tool Usage");
            sb.AppendLine(_toolInsights);
            sb.AppendLine();
        }
    ...
    private void UpdateEditablePromptIfUnchanged()
    {
        string newPreview = GetFullPromptPreview();
        if (string.IsNullOrEmpty(_editableInputPrompt) || _editableInputPrompt == _lastAutoGeneratedPrompt)
        {
            _editableInputPrompt = newPreview;
            _lastAutoGeneratedPrompt = newPreview;
        }
    }
}
